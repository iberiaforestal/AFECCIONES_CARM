...
# === PROCESAR TODAS LAS CAPAS (VP, ZEPA, LIC, ENP) ===
    ...
    # === CONDICIONADO:===
        ...
        margen_lateral = 15
        separacion = 5
        ancho_total = self.w - 2 * margen_lateral
        ancho_columna = (ancho_total - separacion) / 2
        line_h = 4.5
        self.set_font("Arial", "", 9)
        ...
        def altura_parrafo(parrafo):
            h = 0
            for linea in parrafo.split('\n'):
                if linea.strip():
                    w = self.get_string_width(linea)
                    lineas = max(1, math.ceil(w / ancho_columna))
                    h += lineas * line_h
                else:
                    h += line_h
            return h
        ...
        # Altura máxima disponible (desde y actual hasta 30mm del final)
        y_inicio = self.get_y()
        altura_disponible = self.h - y_inicio - 30  # 30mm pie de página

        # Si no cabe en una página → dividir en dos páginas
    ...
    def render_dos_columnas_balanceadas(self, col1, col2, w, h, mx, sep):
        # Dividir cada columna para que quepa
        ...
        # Página 1: col1_izq + col2_izq
        y0 = self.get_y()
        self.set_xy(mx, y0)
        for t, _ in c1_izq: self.multi_cell(w, h, t, align="J")
        y1 = self.get_y()

        self.set_xy(mx + w + sep, y0)
        for t, _ in c2_izq: self.multi_cell(w, h, t, align="J")
        self.set_y(max(y1, self.get_y()))

        # Página 2: col1_der + col2_der
        self.add_page()
        self.set_font("Arial", "B", 12)
        ...
        y0 = self.get_y()
        self.set_xy(mx, y0)
        for t, _ in c1_der: self.multi_cell(w, h, t, align="J")
        y1 = self.get_y()

        self.set_xy(mx + w + sep, y0)
        for t, _ in c2_der: self.multi_cell(w, h, t, align="J")
        self.set_y(max(y1, self.get_y()))
