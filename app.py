...
# === PROCESAR TODAS LAS CAPAS (VP, ZEPA, LIC, ENP, ESTEPARIAS) ===
    ...
    if esteparias_detectado:
        ...
        for cuad, especie, nombre in esteparias_detectado:
            ...
            if pdf.get_y() + row_h > pdf.h - pdf.b_margin: pdf.add_page()
            x, y = pdf.get_x(), pdf.get_y()
            pdf.rect(x, y, col_cuad, row_h); pdf.rect(x+col_cuad, y, col_esp, row_h); pdf.rect(x+col_cuad+col_esp, y, col_nom, row_h)
            pdf.set_xy(x, y + (row_h - cuad_l*line_height)/2); pdf.multi_cell(col_cuad, line_height, str(cuad))
            pdf.set_xy(x+col_cuad, y + (row_h - esp_l*line_height)/2); pdf.multi_cell(col_esp, line_height, str(especie))
