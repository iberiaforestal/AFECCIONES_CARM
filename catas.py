...
# Diccionario de municipios (nombre: código DIR3)
municipios_dir3 = {
    "ABANILLA": 1,
    "ABARAN": 2,
    "AGUILAS": 3,
    ...
    "CIEZA": 19,
    "FORTUNA": 20,
    "FUENTE ALAMO DE MURCIA": 21,
    "JUMILLA": 22,
    "LAS TORRES DE COTILLAS": 23,
    "LA UNION": 24,
    "LIBRILLA": 25,
    "LORCA": 26,
    "LORQUI": 27,
    ...
    "SANTOMERA": 38,
    "SAN JAVIER": 39,
    "SAN PEDRO DEL PINATAR": 40,
    "TORRE PACHECO": 41,
    "TOTANA": 42,
    "ULEA": 43,
    "VILLANUEVA DEL RIO SEGURA": 44,
    "YECLA": 45,
}

# Función para obtener poligonos y parcelas de un municipio (para los desplegables)
def obtener_poligonos_parcelas_municipio(provincia, municipio):
    """
    Devuelve listas de polígonos y parcelas para poblar los desplegables.
    Aquí usamos rangos de ejemplo (001-005 y 00001-00010)
    """
    poligonos = [f"{i:03d}" for i in range(1, 6)]
    parcelas = [f"{i:05d}" for i in range(1, 11)]
    return poligonos, parcelas

# Funciones WCF
def buscar_por_parcela(provincia, municipio, poligono, parcela):
    try:
        response = client.service.Consulta_DNPPP(
            Provincia=str(provincia).zfill(2),
            Municipio=str(municipio).zfill(3),
            Poligono=str(poligono).zfill(5),
            Parcela=str(parcela).zfill(5),
            RefCat=""
        )
        return {
            "RefCat": getattr(response.lrc, "rc", None),
        ...
        st.error(f"Error WCF Consulta_DNPPP: {str(e)}")
        return None
...
def buscar_por_coordenadas(lat, lon):
    try:
        ...
        st.error(f"Error WCF Consulta_RCCOOR: {str(e)}")
        return None

# -----------------------------
# NUEVA FUNCIÓN: Obtener geometría
# -----------------------------
from shapely import wkt

def obtener_geometria_parcela(provincia, municipio, poligono, parcela):
    try:
        response = client.service.Consulta_BIS(
            Provincia=str(provincia).zfill(2),
            Municipio=str(municipio).zfill(3),
            Poligono=str(poligono).zfill(5),
            Parcela=str(parcela).zfill(5),
        ...
        geom_wkt = getattr(response, "geom", None)
        if geom_wkt:
            poligono = wkt.loads(geom_wkt)
            centroide = poligono.centroid
            return poligono, centroide.x, centroide.y
        else:
            return None, 0.0, 0.0
    except Exception as e:
        st.error(f"Error al obtener geometría de la parcela: {str(e)}")
        return None, 0.0, 0.0
...
if modo == "Por parcela":
    st.subheader("Búsqueda por Parcela")

    # Desplegable de municipios
    municipio_sel = st.selectbox("Municipio", sorted(municipios_dir3.keys()))
    provincia_code = 30  # Por ejemplo, Murcia
    municipio_code = municipios_dir3[municipio_sel]

    # Desplegables de polígono y parcela usando la función de ejemplo
    poligonos, parcelas = obtener_poligonos_parcelas_municipio(provincia_code, municipio_code)
    poligono_sel = st.selectbox("Polígono", poligonos)
    parcela_sel = st.selectbox("Parcela", parcelas)

    if st.button("Buscar Parcela"):
        # Buscar datos de la parcela con WCF
        resultado = buscar_por_parcela(provincia_code, municipio_code, poligono_sel, parcela_sel)

        if resultado and resultado["RefCat"]:
            st.success("Parcela encontrada")
            st.write(f"RefCat: {resultado['RefCat']}")
            st.write(f"Provincia: {resultado['Provincia']}")
            st.write(f"Municipio: {resultado['Municipio']}")
            st.write(f"Polígono: {resultado['Poligono']}")
            st.write(f"Parcela: {resultado['Parcela']}")

            # Obtener geometría real y centroide
            poligono_geom, x, y = obtener_geometria_parcela(provincia_code, municipio_code, poligono_sel, parcela_sel)
            if poligono_geom:
                st.info(f"Coordenadas del centroide de la parcela: X = {x}, Y = {y}")
            else:
                st.warning("No se pudo obtener la geometría de la parcela.")
        else:
            st.error("No se encontró la parcela con los datos proporcionados.")

with st.form("formulario"):
    if modo == "Por coordenadas":
        x = st.number_input("Latitud (EPSG:4326)", value=0.0, format="%.6f", help="Introduce coordenadas en WGS84")
        y = st.number_input("Longitud (EPSG:4326)", value=0.0, format="%.6f")
        
        if st.form_submit_button("Buscar por Coordenadas"):
            if x != 0.0 and y != 0.0:
                resultado = buscar_por_coordenadas(x, y)
                if resultado and resultado["RefCat"]:
                    st.success("Parcela encontrada")
                    st.write(f"RefCat: {resultado['RefCat']}")
                    st.write(f"Provincia: {resultado['Provincia']}")
                    ...
                    # Obtener geometría real y centroide
                    poligono_geom, centroide_x, centroide_y = obtener_geometria_parcela(
                        resultado["Provincia"],
                        resultado["Municipio"],
                        resultado["Polígono"],
                        resultado["Parcela"]
                    ...
                    if poligono_geom:
                        st.info(f"Coordenadas del centroide de la parcela: X = {centroide_x}, Y = {centroide_y}")
                    else:
                        st.warning("No se pudo obtener la geometría de la parcela.")
                else:
